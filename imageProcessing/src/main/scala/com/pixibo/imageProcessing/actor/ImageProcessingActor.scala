package com.pixibo.imageProcessing.actor
...
import com.pixibo.imageProcessing.Helper.Utils

class ImageProcessingActor() extends Actor {
  ...
  def receive = {
     ...
     */
    case Request(uri, operations) => {
      Utils.downloadFileFromURL(uri) match {
        case Right(res) => {
          val image = Image(res.getResponseBodyAsBytes)
